...
class UseModelPyTorch: # Only works for loading PyTorch models - other models will be loadable soon 
    ...
    def loadTargetlessInput(self, data_point):
        self.features = torch.tensor(data_point, dtype=torch.float32).to(self.device).unsqueeze(0)
        
        self.input_targetless = True
        
    def loadInput(self, data_point): # Input 2D array - each internal array is a single datapoint
...
class UseModelSklearn: # Only supports supervised ML models (so not Isolation Forest)
    def __init__(self, model_file):
        self.loadModel(model_file)
    ...
    def loadTargetlessInput(self, data_points):
        # Ensure that input is a 2D array (each row is a separate data point)
        self.input_features = np.array(data_points)  
        self.input_targetless = False
            
    def loadInput(self, data_points): 
        """Load one or more input data points for prediction."""
        # Ensure that input is a 2D array (each row is a separate data point)
    ...
    def predictOnInput(self):
        """Make a prediction on one or more inputs."""
