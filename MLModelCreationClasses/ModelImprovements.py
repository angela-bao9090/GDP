...
def plotGraphTest(modelFunc, name):  # to test the final choices for the model #takes in list of tuples names and models
    ...
    f = f1Calc(p, r)
    print(name + ": " + "precision: " + "{:.2f}".format(p),
          "recall: " + "{:.2f}".format(r) + " F1: " + "{:.2f}".format(f))
    return p, r, f
...
def genLogisticRegressionGraph():
    ...
    plotGraph(100, 300, False, lambda m: Models.LogisticRegressionModel(train_file, test_file, max_iter=m),
              ...
              "Max Iterations")  # must take in int our graphing function comes up with floats
...
def testAllOptimal():
    ...
    # random Forest
    p, r, f = plotGraphTest(Models.RandomForestModel(train_file, test_file, max_depth=30, threshold=0.7),
                            "Random Forest")
    precision.append(p)
    recall.append(r)
    f1.append(f)
